// Package tui — localized UI strings and slash-command definitions.
package tui

import (
	"os"
	"strings"
)

const (
	langZhCN = "zh-CN"
	langZhTW = "zh-TW"
	langEn   = "en"
)

var tuiStrings = map[string]map[string]string{
	langZhCN: {
		"ac.title":             "命令提示",
		"ac.hint":              "↑↓ 选择 · Tab 补全 · Esc 关闭",
		"ac.custom":            "自定义命令",
		"ac.arg":               "参数值",
		"banner.hint":          "输入你的需求，或 /help 查看命令。按 Ctrl+C 退出。",
		"welcome.hint":         "输入你的需求开始对话，或 /help 查看全部命令 · Tab 补全 · ↑↓ 历史",
		"welcome.tagline":      "你的 AI 编程伙伴",
		"welcome.close":        "按 Esc 或回车关闭欢迎屏 · 输入即开始",
		"welcome.reopen":       "欢迎屏仅在空对话时显示，先 /clear 再试",
		"input.hint":           "manual mode on · ? for shortcuts · Enter for agents",
		"input.hint.streaming": "esc 中断生成",
		"cmd.welcome":          "显示/隐藏启动欢迎屏",
		"perm.title":           "需要授权",
		"perm.allow":           "允许",
		"perm.all":             "本次会话全部允许",
		"perm.deny":            "拒绝",
		"status.gen":           "生成中…",
		"lang.usage":           "用法: /lang <zh-CN|zh-TW|en>",
		"lang.set":             "语言已设为 %s",
		"lang.current":         "当前语言: %s",
		"lang.modelNote":       "模型回复、思考过程与代码注释语言已同步切换。",
		"theme.usage":          "用法: /theme <auto|dark|light>",
		"theme.set":            "主题已设为 %s（已持久化）",
		"sc.title":             "快捷键",
		"sc.shell":             "运行 shell 命令",
		"sc.ctrlc":             "中断 / 退出",
		"sc.ctrll":             "清屏",
		"sc.history":           "历史记录",
		"cmd.help":             "显示本帮助",
		"cmd.model":            "切换模型",
		"cmd.provider":         "切换提供商",
		"cmd.mode":             "切换权限模式",
		"cmd.cd":               "移动会话工作目录",
		"cmd.plan":             "切换到 plan 模式（只读规划）",
		"cmd.ask":              "切换到 ask 模式（只读提问）",
		"cmd.debug":            "切换到 agent 模式（调试/动手执行）",
		"cmd.rename":           "重命名当前会话",
		"cmd.branch":           "从历史会话分支出一个独立会话（/fork 别名）",
		"cmd.checkpoint":       "回滚前 N 步工具调用（/rewind 别名）",
		"cmd.usage":            "显示 Token 与成本明细（/cost 别名）",
		"cmd.stats":            "显示 Token 节省报告（/token 别名）",
		"cmd.keys":             "显示各提供商 API Key 状态",
		"cmd.models":           "显示自定义模型",
		"cmd.config":           "显示当前设置",
		"cmd.history":          "显示输入历史",
		"cmd.sessions":         "列出已保存会话",
		"cmd.resume":           "恢复指定会话",
		"cmd.restore":          "恢复被 /clear 软删除的会话",
		"cmd.compact":          "压缩上下文（摘要 + 保留近期）",
		"cmd.clear":            "清空当前对话",
		"cmd.export":           "导出对话为 Markdown",
		"cmd.share":            "导出可分享 Markdown（时间戳文件并打印路径）",
		"cmd.diff":             "显示 git diff",
		"cmd.lsp":              "LSP 代码智能（诊断/符号/定义/引用）",
		"cmd.kb":               "搜索本地知识库（/kb <查询>）",
		"cmd.todo":             "显示会话待办列表",
		"cmd.rewind":           "回滚前 N 步工具调用",
		"cmd.undo":             "撤销最近一次工具调用（回滚 1 步）",
		"cmd.apply":            "应用所有已暂存的搜索替换编辑",
		"cmd.reject":           "丢弃所有已暂存的编辑",
		"cmd.new":              "创建新会话",
		"cmd.search":           "搜索历史对话",
		"cmd.admin":            "切换管理员模式（yolo/ask）",
		"cmd.outputstyle":      "设置回答风格（concise 简洁 / normal 标准 / verbose 详细）",
		"cmd.update":           "刷新模型目录（从各提供商拉取最新模型清单）",
		"cmd.adddir":           "添加额外工作目录（注入上下文）",
		"cmd.init":             "生成项目 ICODE.md 骨架",
		"cmd.agents":           "列出子 agent",
		"cmd.skills":           "列出可用技能 (SKILL.md)",
		"cmd.skillEval":        "技能触发准确率自测",
		"cmd.plugin":           "插件管理 (list/install/remove)",
		"cmd.teams":            "列出多智能体团队",
		"cmd.mcp":              "管理 MCP 服务器 (list/add/remove/get/restart)",
		"cmd.hooks":            "编辑 hooks.yaml 权限规则",
		"cmd.status":           "显示系统状态诊断",
		"cmd.cost":             "显示 Token 与成本明细",
		"cmd.token":            "查看本会话 Token 节省报告（Cache-First 五层压缩明细）",
		"cmd.theme":            "设置主题",
		"cmd.lang":             "设置语言",
		"cmd.exit":             "退出 iCode",
		"cmd.ac":               "(输入 / 时) ↑↓ 选择命令 · Tab 补全 · Esc 关闭",
		"cmd.summarize":        "总结当前对话内容",
		"cmd.fork":             "从历史会话分支出一个独立会话",
		"cmd.goal":             "长目标模式：设置/查看/清除会话目标（每轮自动携带）",
		"cmd.budget":           "Token 预算护栏：设置/查看/清除会话 token 上限（超限自动压缩）",
		"cmd.review":           "审查代码 (git diff 或指定文件)",
		"cmd.security":         "显示/切换安全等级（本地处理 · 脱敏处理 · 本地大模型 · 国外大模型 · 无限制）",
		"cmd.expand":           "展开/折叠工具输出详情",
		"cmd.doctor":           "运行系统诊断（配置/提供商/记忆文件）",
		"cmd.whoami":           "显示当前身份（模型/提供商/版本）",
		"cmd.context":          "显示上下文窗口用量",
		"cmd.permissions":      "显示当前权限/安全等级",
		"cmd.verbose":          "切换详细输出（完整工具参数/原始 diff）",
		"cmd.memory":           "显示记忆文件路径",
		"cmd.feedback":         "显示反馈渠道信息",
		"cmd.wipe":             "清空对话并重置（不可恢复）",
		"cmd.login":            "配置 API 凭据（见提示）",
		"cmd.logout":           "清除已保存的 API 凭据",
		"cmd.copy":             "复制最近一条助手回复到剪贴板 (/copy [N])",
		"security.usage":       "用法: /security [local|desensitize|local-llm|foreign-llm|unrestricted]\n当前等级: %s\n\n等级说明:\n  local          [L] 所有数据仅在本地处理，不调用外部 API\n  desensitize    [D] 发送前脱敏（隐藏身份证/手机号/密钥等）\n  local-llm      [M] 仅允许本地模型（Ollama/Llama.cpp 等）\n  foreign-llm    [G] 允许国外大模型 API\n  unrestricted   [!] 无安全限制\n",
		"security.set":         "安全等级已设为: %s",
		"security.desc":        "安全等级 — 控制数据隐私边界",
		"settings.title":       " 设置 (Esc 关闭) ",
		"settings.model":       "模型",
		"settings.provider":    "提供商",
		"settings.mode":        "权限模式",
		"settings.lang":        "语言",
		"settings.theme":       "主题",
		"settings.voiceProv":   "语音提供商",
		"settings.baiduKey":    "百度 API Key",
		"settings.xfyunAppID":  "讯飞 App ID",
		"settings.soon":        "设置修改功能即将推出，请使用 /config 命令修改设置。",
	},
	langZhTW: {
		"ac.title":             "指令提示",
		"ac.hint":              "↑↓ 選擇 · Tab 補全 · Esc 關閉",
		"ac.custom":            "自訂指令",
		"ac.arg":               "參數值",
		"banner.hint":          "輸入你的需求，或 /help 檢視指令。按 Ctrl+C 退出。",
		"welcome.hint":         "輸入你的需求開始對話，或 /help 檢視全部指令 · Tab 補全 · ↑↓ 歷史",
		"welcome.tagline":      "你的 AI 程式設計夥伴",
		"welcome.close":        "按 Esc 或 Enter 關閉歡迎屏 · 輸入即開始",
		"welcome.reopen":       "歡迎屏僅在空對話時顯示，先 /clear 再試",
		"input.hint":           "manual mode on · ? for shortcuts · Enter for agents",
		"input.hint.streaming": "esc 中斷生成",
		"cmd.welcome":          "顯示/隱藏啟動歡迎屏",
		"perm.title":           "需要授權",
		"perm.allow":           "允許",
		"perm.all":             "本次工作階段全部允許",
		"perm.deny":            "拒絕",
		"status.gen":           "生成中…",
		"cmd.skillEval":        "技能觸發準確率自測",
		"cmd.plugin":           "外掛管理 (list/install/remove)",
		"lang.usage":           "用法: /lang <zh-CN|zh-TW|en>",
		"lang.set":             "語言已設為 %s",
		"lang.current":         "目前語言: %s",
		"lang.modelNote":       "模型回覆、思考過程與程式碼註解語言已同步切換。",
		"theme.usage":          "用法: /theme <auto|dark|light>",
		"theme.set":            "主題已設為 %s（已儲存）",
		"sc.title":             "快捷鍵",
		"sc.shell":             "執行 shell 指令",
		"sc.ctrlc":             "中斷 / 退出",
		"sc.ctrll":             "清螢幕",
		"sc.history":           "歷史紀錄",
		"cmd.help":             "顯示說明",
		"cmd.model":            "切換模型",
		"cmd.provider":         "切換提供商",
		"cmd.mode":             "切換權限模式",
		"cmd.cd":               "移動工作階段工作目錄",
		"cmd.plan":             "切換到 plan 模式（唯讀規劃）",
		"cmd.ask":              "切換到 ask 模式（唯讀提問）",
		"cmd.debug":            "切換到 agent 模式（除錯/動手執行）",
		"cmd.rename":           "重新命名目前工作階段",
		"cmd.branch":           "從歷史工作階段分支出獨立工作階段（/fork 別名）",
		"cmd.checkpoint":       "回滾前 N 步工具呼叫（/rewind 別名）",
		"cmd.usage":            "顯示 Token 與成本明細（/cost 別名）",
		"cmd.stats":            "顯示 Token 節省報告（/token 別名）",
		"cmd.keys":             "顯示各提供商 API 金鑰狀態",
		"cmd.models":           "顯示自訂模型",
		"cmd.config":           "顯示目前設定",
		"cmd.history":          "顯示輸入歷史",
		"cmd.sessions":         "列出已儲存工作階段",
		"cmd.resume":           "恢復指定工作階段",
		"cmd.restore":          "恢復被 /clear 軟刪除的工作階段",
		"cmd.compact":          "壓縮上下文（摘要 + 保留近期）",
		"cmd.clear":            "清空目前對話",
		"cmd.export":           "匯出對話為 Markdown",
		"cmd.share":            "匯出可分享 Markdown（時間戳檔案並列印路徑）",
		"cmd.diff":             "顯示 git diff",
		"cmd.lsp":              "LSP 程式碼智慧（診斷/符號/定義/引用）",
		"cmd.kb":               "搜尋本機知識庫（/kb <查詢>）",
		"cmd.todo":             "顯示工作階段待辦清單",
		"cmd.rewind":           "回滾前 N 步工具呼叫",
		"cmd.undo":             "復原最近一次工具呼叫（回滾 1 步）",
		"cmd.apply":            "套用所有已暫存的搜尋取代編輯",
		"cmd.reject":           "捨棄所有已暫存的編輯",
		"cmd.new":              "建立新工作階段",
		"cmd.search":           "搜尋歷史對話",
		"cmd.admin":            "切換管理員模式（yolo/ask）",
		"cmd.outputstyle":      "設定回答風格（concise 精簡 / normal 標準 / verbose 詳細）",
		"cmd.update":           "重新整理模型目錄（從各提供商拉取最新模型清單）",
		"cmd.adddir":           "新增額外工作目錄（注入上下文）",
		"cmd.init":             "產生專案 ICODE.md 骨架",
		"cmd.agents":           "列出子 agent",
		"cmd.mcp":              "管理 MCP 伺服器 (list/add/remove/get/restart)",
		"cmd.hooks":            "編輯 hooks.yaml 權限規則",
		"cmd.status":           "顯示系統狀態診斷",
		"cmd.cost":             "顯示 Token 與成本明細",
		"cmd.token":            "查看本會話 Token 節省報告（Cache-First 五層壓縮明細）",
		"cmd.theme":            "設定主題",
		"cmd.lang":             "設定語言",
		"cmd.exit":             "退出 iCode",
		"cmd.ac":               "(輸入 / 時) ↑↓ 選擇指令 · Tab 補全 · Esc 關閉",
		"cmd.summarize":        "總結目前對話內容",
		"cmd.fork":             "從歷史對話分支出一個獨立對話",
		"cmd.goal":             "長目標模式：設定/檢視/清除對話目標（每輪自動攜帶）",
		"cmd.budget":           "Token 預算護欄：設定/檢視/清除對話 token 上限（超限自動壓縮）",
		"cmd.review":           "審查程式碼 (git diff 或指定檔案)",
		"cmd.security":         "顯示/切換安全等級（本地處理 · 脫敏處理 · 本地大模型 · 國外大模型 · 無限制）",
		"cmd.expand":           "展開/折疊工具輸出詳情",
		"cmd.doctor":           "執行系統診斷（設定/提供商/記憶檔案）",
		"cmd.whoami":           "顯示目前身分（模型/提供商/版本）",
		"cmd.context":          "顯示上下文視窗用量",
		"cmd.permissions":      "顯示目前權限/安全等級",
		"cmd.verbose":          "切換詳細輸出（完整工具參數/原始 diff）",
		"cmd.memory":           "顯示記憶檔案路徑",
		"cmd.feedback":         "顯示回饋管道資訊",
		"cmd.wipe":             "清空對話並重置（不可復原）",
		"cmd.login":            "設定 API 憑證（見提示）",
		"cmd.logout":           "清除已儲存的 API 憑證",
		"cmd.copy":             "複製最近一條助手回覆到剪貼簿 (/copy [N])",
		"security.usage":       "用法: /security [local|desensitize|local-llm|foreign-llm|unrestricted]\n目前等級: %s\n\n等級說明:\n  local          [L] 所有資料僅在本機處理，不呼叫外部 API\n  desensitize    [D] 傳送前脫敏（隱藏身分證/手機號/金鑰等）\n  local-llm      [M] 僅允許本機模型（Ollama/Llama.cpp 等）\n  foreign-llm    [G] 允許國外大模型 API\n  unrestricted   [!] 無安全限制\n",
		"security.set":         "安全等級已設為: %s",
		"security.desc":        "安全等級 — 控制資料隱私邊界",
		"settings.title":       " 設定 (Esc 關閉) ",
		"settings.model":       "模型",
		"settings.provider":    "提供商",
		"settings.mode":        "權限模式",
		"settings.lang":        "語言",
		"settings.theme":       "主題",
		"settings.voiceProv":   "語音提供商",
		"settings.baiduKey":    "百度 API Key",
		"settings.xfyunAppID":  "訊飛 App ID",
		"settings.soon":        "設定修改功能即將推出，請使用 /config 指令修改設定。",
	},
	langEn: {
		"ac.title":             "Commands",
		"ac.hint":              "↑↓ to move · Tab to complete · Esc to close",
		"ac.custom":            "custom command",
		"ac.arg":               "value",
		"banner.hint":          "Type your request, or /help for commands. Ctrl+C to exit.",
		"welcome.hint":         "Type your request to start, or /help for all commands · Tab to complete · ↑↓ history",
		"welcome.tagline":      "your AI coding partner",
		"welcome.close":        "Press Esc or Enter to close · just start typing",
		"welcome.reopen":       "Welcome shows only on an empty chat — run /clear first",
		"input.hint":           "manual mode on · ? for shortcuts · Enter for agents",
		"input.hint.streaming": "esc to interrupt",
		"cmd.welcome":          "Show/hide the startup welcome screen",
		"perm.title":           "Needs approval",
		"perm.allow":           "allow",
		"perm.all":             "allow all this session",
		"perm.deny":            "deny",
		"status.gen":           "generating…",
		"lang.usage":           "usage: /lang <zh-CN|zh-TW|en>",
		"cmd.skillEval":        "技能觸發準確率自測",
		"cmd.plugin":           "外掛管理 (list/install/remove)",
		"lang.set":             "Language set to %s",
		"lang.current":         "Current language: %s",
		"lang.modelNote":       "Model replies, reasoning, and code comments now follow this language.",
		"theme.usage":          "usage: /theme <auto|dark|light>",
		"theme.set":            "Theme set to %s (saved)",
		"sc.title":             "Shortcuts",
		"sc.shell":             "run a shell command",
		"sc.ctrlc":             "interrupt / exit",
		"sc.ctrll":             "clear screen",
		"sc.history":           "history",
		"cmd.help":             "Show this help",
		"cmd.model":            "Switch model",
		"cmd.provider":         "Switch provider",
		"cmd.mode":             "Switch permission mode",
		"cmd.cd":               "Move the session working directory",
		"cmd.plan":             "Switch to plan mode (read-only planning)",
		"cmd.ask":              "Switch to ask mode (read-only questions)",
		"cmd.debug":            "Switch to agent mode (debug / hands-on)",
		"cmd.rename":           "Rename the current session",
		"cmd.branch":           "Branch a new session from history (/fork alias)",
		"cmd.checkpoint":       "Roll back N tool calls (/rewind alias)",
		"cmd.usage":            "Show token & cost breakdown (/cost alias)",
		"cmd.stats":            "Show token savings report (/token alias)",
		"cmd.keys":             "Show API key status per provider",
		"cmd.models":           "Show custom models",
		"cmd.config":           "Show current settings",
		"cmd.history":          "Show input history",
		"cmd.sessions":         "List saved sessions",
		"cmd.resume":           "Resume a session",
		"cmd.restore":          "Restore a soft-deleted session (from /clear)",
		"cmd.fork":             "Branch an independent session from history",
		"cmd.goal":             "Long-goal mode: set/show/clear the session goal (auto-injected each turn)",
		"cmd.budget":           "Token budget guard: set/show/clear the session token cap (auto-shrinks on exceed)",
		"cmd.compact":          "Compact context (summarize & keep recent)",
		"cmd.clear":            "Clear conversation",
		"cmd.export":           "Export conversation to Markdown",
		"cmd.share":            "Export shareable Markdown (timestamped file + path)",
		"cmd.diff":             "Show git diff",
		"cmd.lsp":              "LSP code intelligence (diagnostics/symbols/defs/refs)",
		"cmd.kb":               "Search local knowledge base (/kb <query>)",
		"cmd.todo":             "Show session todo list",
		"cmd.rewind":           "Rewind N tool steps",
		"cmd.undo":             "Undo the last tool step (rewind 1)",
		"cmd.apply":            "Apply all staged search-replace edits",
		"cmd.reject":           "Discard all staged edits",
		"cmd.new":              "Create a new session",
		"cmd.search":           "Search past conversations",
		"cmd.admin":            "Toggle admin mode (yolo/ask)",
		"cmd.outputstyle":      "Set answer style (concise / normal / verbose)",
		"cmd.update":           "Refresh model catalogs from all providers",
		"cmd.adddir":           "Add an extra working directory (inject into context)",
		"cmd.init":             "Generate ICODE.md project skeleton",
		"cmd.agents":           "List sub-agents",
		"cmd.mcp":              "Manage MCP servers (list/add/remove/get/restart)",
		"cmd.hooks":            "Edit hooks.yaml permission rules",
		"cmd.status":           "Show system diagnostics",
		"cmd.cost":             "Show token & cost breakdown",
		"cmd.token":            "Show token savings report (Cache-First 5-layer pipeline)",
		"cmd.theme":            "Set theme",
		"cmd.lang":             "Set language",
		"cmd.exit":             "Exit iCode",
		"cmd.ac":               "(typing / ) ↑↓ to choose · Tab to complete · Esc to close",
		"cmd.summarize":        "Summarize the current conversation",
		"cmd.review":           "Review code (git diff or a file)",
		"cmd.security":         "Show/switch security level (local · desensitize · local-llm · foreign-llm · unrestricted)",
		"cmd.expand":           "Expand/collapse tool output details",
		"cmd.doctor":           "Run system diagnostics (config/providers/memory)",
		"cmd.whoami":           "Show current identity (model/provider/version)",
		"cmd.context":          "Show context-window usage",
		"cmd.permissions":      "Show current permission/security level",
		"cmd.verbose":          "Toggle verbose output (full tool args / raw diffs)",
		"cmd.memory":           "Show memory file paths",
		"cmd.feedback":         "Show feedback channel info",
		"cmd.wipe":             "Clear conversation and reset (irreversible)",
		"cmd.login":            "Configure API credentials (see hint)",
		"cmd.logout":           "Clear saved API credentials",
		"cmd.copy":             "Copy last assistant reply to clipboard (/copy [N])",
		"cmd.vim":              "Toggle vim key bindings (vi-style input)",
		"cmd.statusline":       "Toggle the bottom status bar",
		"cmd.prcomments":       "Show pull-request comments (requires gh)",
		"cmd.releasenotes":     "Show the latest release notes",
		"cmd.bug":              "Open a pre-filled GitHub bug-report issue",
		"security.usage":       "Usage: /security [local|desensitize|local-llm|foreign-llm|unrestricted]\nCurrent level: %s\n\nLevels:\n  local          [L] All data stays local, no external API calls\n  desensitize    [D] PII redacted before sending (IDs/phones/keys)\n  local-llm      [M] Only local models allowed (Ollama/Llama.cpp etc.)\n  foreign-llm    [G] Foreign LLM APIs allowed\n  unrestricted   [!] No security restrictions\n",
		"security.set":         "Security level set to: %s",
		"security.desc":        "Security level — controls data privacy boundary",
		"settings.title":       " Settings (Esc to close) ",
		"settings.model":       "Model",
		"settings.provider":    "Provider",
		"settings.mode":        "Mode",
		"settings.lang":        "Language",
		"settings.theme":       "Theme",
		"settings.voiceProv":   "Voice provider",
		"settings.baiduKey":    "Baidu API Key",
		"settings.xfyunAppID":  "iFlytek App ID",
		"settings.soon":        "Editing settings here is coming soon — use /config to change settings.",
	},
}

var slashDefs = []struct {
	Name string
	Key  string
}{
	{"/help", "cmd.help"},
	{"/cd", "cmd.cd"},
	{"/model", "cmd.model"},
	{"/provider", "cmd.provider"},
	{"/mode", "cmd.mode"},
	{"/plan", "cmd.plan"},
	{"/ask", "cmd.ask"},
	{"/debug", "cmd.debug"},
	{"/keys", "cmd.keys"},
	{"/models", "cmd.models"},
	{"/config", "cmd.config"},
	{"/history", "cmd.history"},
	{"/sessions", "cmd.sessions"},
	{"/resume", "cmd.resume"},
	{"/restore", "cmd.restore"},
	{"/fork", "cmd.fork"},
	{"/branch", "cmd.branch"},
	{"/rename", "cmd.rename"},
	{"/goal", "cmd.goal"},
	{"/budget", "cmd.budget"},
	{"/compact", "cmd.compact"},
	{"/clear", "cmd.clear"},
	{"/export", "cmd.export"},
	{"/share", "cmd.share"},
	{"/diff", "cmd.diff"},
	{"/lsp", "cmd.lsp"},
	{"/kb", "cmd.kb"},
	{"/todo", "cmd.todo"},
	{"/rewind", "cmd.rewind"},
	{"/undo", "cmd.undo"},
	{"/checkpoint", "cmd.checkpoint"},
	{"/apply", "cmd.apply"},
	{"/reject", "cmd.reject"},
	{"/new", "cmd.new"},
	{"/search", "cmd.search"},
	{"/admin", "cmd.admin"},
	{"/output-style", "cmd.outputstyle"},
	{"/update", "cmd.update"},
	{"/add-dir", "cmd.adddir"},
	{"/init", "cmd.init"},
	{"/agents", "cmd.agents"},
	{"/skills", "cmd.skills"},
	{"/skill-eval", "cmd.skillEval"},
	{"/plugin", "cmd.plugin"},
	{"/teams", "cmd.teams"},
	{"/mcp", "cmd.mcp"},
	{"/hooks", "cmd.hooks"},
	{"/status", "cmd.status"},
	{"/cost", "cmd.cost"},
	{"/usage", "cmd.usage"},
	{"/stats", "cmd.stats"},
	{"/token", "cmd.token"},
	{"/theme", "cmd.theme"},
	{"/lang", "cmd.lang"},
	{"/welcome", "cmd.welcome"},
	{"/security", "cmd.security"},
	{"/summarize", "cmd.summarize"},
	{"/review", "cmd.review"},
	{"/exit", "cmd.exit"},
	{"/expand", "cmd.expand"},
	{"/doctor", "cmd.doctor"},
	{"/whoami", "cmd.whoami"},
	{"/context", "cmd.context"},
	{"/permissions", "cmd.permissions"},
	{"/verbose", "cmd.verbose"},
	{"/memory", "cmd.memory"},
	{"/feedback", "cmd.feedback"},
	{"/wipe", "cmd.wipe"},
	{"/login", "cmd.login"},
	{"/logout", "cmd.logout"},
	{"/vim", "cmd.vim"},
	{"/statusline", "cmd.statusline"},
	{"/pr_comments", "cmd.prcomments"},
	{"/release-notes", "cmd.releasenotes"},
	{"/bug", "cmd.bug"},
	{"/copy", "cmd.copy"},
}

type acItem struct {
	Name string
	Desc string
	// ArgPrefix, when set, marks this as an argument-completion entry for the
	// slash command in the prefix (e.g. "/model"); accepting it replaces only
	// the argument part of the input buffer.
	ArgPrefix string
}

var palettes = map[string]map[string]string{
	"dark": {
		"dim":          "\x1b[90m",
		"bold":         "\x1b[1m",
		"red":          "\x1b[31m",
		"green":        "\x1b[32m",
		"yellow":       "\x1b[33m",
		"brightyellow": "\x1b[38;5;220m", // 大黄 (golden yellow ~#FFD75F, iCODE dot)
		"orange":       "\x1b[38;5;209m", // brand orange (~#ff7a45)
		"blue":         "\x1b[34m",
		"magenta":      "\x1b[38;5;205m",
		"purple":       "\x1b[38;5;141m", // brighter violet for better contrast
		"cyan":         "\x1b[36m",
		"white":        "\x1b[37m",
	},
	"light": {
		"dim":          "\x1b[90m",
		"bold":         "\x1b[1m",
		"red":          "\x1b[31m",
		"green":        "\x1b[38;5;28m",
		"yellow":       "\x1b[38;5;136m",
		"brightyellow": "\x1b[38;5;178m", // deep golden yellow on light bg (~#D7AF00)
		"orange":       "\x1b[38;5;166m",
		"blue":         "\x1b[34m",
		"magenta":      "\x1b[35m",
		"purple":       "\x1b[38;5;129m",
		"cyan":         "\x1b[38;5;30m",
		"white":        "\x1b[30m",
	},
}

func (t *TUI) tstr(key string) string {
	lang := t.lang
	if lang == "" {
		lang = langZhCN
	}
	if m, ok := tuiStrings[lang]; ok {
		if v, ok := m[key]; ok && v != "" {
			return v
		}
	}
	if m, ok := tuiStrings[langZhCN]; ok {
		if v, ok := m[key]; ok {
			return v
		}
	}
	return key
}

func (t *TUI) activeTheme() string {
	if t.theme == "light" {
		return "light"
	}
	return "dark"
}

func (t *TUI) c(name string) string {
	if !t.color {
		return ""
	}
	if trueColorTerm() {
		if code, ok := trueColorOverrides[name]; ok {
			return code
		}
	}
	if p, ok := palettes[t.activeTheme()]; ok {
		if code, ok := p[name]; ok {
			return code
		}
	}
	if p, ok := palettes["dark"]; ok {
		if code, ok := p[name]; ok {
			return code
		}
	}
	return ""
}

// trueColorOverrides promotes just the brand hues to 24-bit RGB when the
// terminal advertises truecolor. Everything else stays in the universally-safe
// 256/16-colour codes, so an 8-bit terminal never renders garbage — it simply
// keeps the fallback palette.
var trueColorOverrides = map[string]string{
	"orange":  "\x1b[38;2;255;122;69m",  // brand orange ~#ff7a45
	"purple":  "\x1b[38;2;139;108;255m", // brighter violet ~#8b6cff
	"magenta": "\x1b[38;2;255;94;190m",  // warm pink
}

// trueColorTerm reports whether the terminal supports 24-bit colour, decided
// via COLORTERM. iCode never auto-enables it; it only enhances existing colour
// when the environment explicitly advertises support.
func trueColorTerm() bool {
	ct := strings.ToLower(os.Getenv("COLORTERM"))
	return ct != "" && (strings.Contains(ct, "truecolor") || strings.Contains(ct, "24bit"))
}

func (t *TUI) paint(name, text string) string {
	if !t.color {
		return text
	}
	return t.c(name) + text + "\x1b[0m"
}
