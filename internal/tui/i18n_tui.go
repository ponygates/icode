...
var tuiStrings = map[string]map[string]string{
	langZhCN: {
		...
		"input.hint":     "manual mode on · ? for shortcuts · ↵ for agents",
		...
		"security.usage": "用法: /security [local|desensitize|local-llm|foreign-llm|unrestricted]\n当前等级: %s\n\n等级说明:\n  local          🔒 所有数据仅在本地处理，不调用外部 API\n  desensitize    🛡 发送前脱敏（隐藏身份证/手机号/密钥等）\n  local-llm      💻 仅允许本地模型（Ollama/Llama.cpp 等）\n  foreign-llm    🌐 允许国外大模型 API\n  unrestricted   ⚠ 无安全限制\n",
	...
	langZhTW: {
		...
		"security.usage": "用法: /security [local|desensitize|local-llm|foreign-llm|unrestricted]\n目前等級: %s\n\n等級說明:\n  local          🔒 所有資料僅在本機處理，不呼叫外部 API\n  desensitize    🛡 傳送前脫敏（隱藏身分證/手機號/金鑰等）\n  local-llm      💻 僅允許本機模型（Ollama/Llama.cpp 等）\n  foreign-llm    🌐 允許國外大模型 API\n  unrestricted   ⚠ 無安全限制\n",
	...
	langEn: {
		...
		"security.usage": "Usage: /security [local|desensitize|local-llm|foreign-llm|unrestricted]\nCurrent level: %s\n\nLevels:\n  local          🔒 All data stays local, no external API calls\n  desensitize    🛡 PII redacted before sending (IDs/phones/keys)\n  local-llm      💻 Only local models allowed (Ollama/Llama.cpp etc.)\n  foreign-llm    🌐 Foreign LLM APIs allowed\n  unrestricted   ⚠ No security restrictions\n",
