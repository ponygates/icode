...
func TestNew(t *testing.T) {
	...
	if !ok {
		t.Fatal("Huawei provider should wrap openai_compat.BaseProvider")
	}
...
func TestNew_CustomBase(t *testing.T) {
	customBase := "https://custom.huawei.com"
...
func TestDefaultModels(t *testing.T) {
	...
	if !ids["pangu-5.0-pro"] {
		t.Error("expected pangu-5.0-pro in default models")
	}
	if !ids["pangu-5.0-code"] {
		t.Error("expected pangu-5.0-code in default models")
	}
}
...
func TestProviderViaNew(t *testing.T) {
	p := New("sk-test", "")
	if p.Name() != "huawei" {
		t.Errorf("expected name 'huawei', got %q", p.Name())
	}
	models := p.ListModels()
	if len(models) == 0 {
