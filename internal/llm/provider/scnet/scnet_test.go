...
func TestNew(t *testing.T) {
	...
	if !ok {
		t.Fatal("SCNET provider should wrap openai_compat.BaseProvider")
	}
...
func TestNew_CustomBase(t *testing.T) {
	customBase := "https://custom.scnet.cn"
...
func TestDefaultModels(t *testing.T) {
	...
	if !ids["scnet-chat"] {
		t.Error("expected scnet-chat in default models")
	}
	if !ids["scnet-code"] {
		t.Error("expected scnet-code in default models")
	}
...
func TestProviderViaNew(t *testing.T) {
	p := New("sk-test", "")
	if p.Name() != "scnet" {
		t.Errorf("expected name 'scnet', got %q", p.Name())
	}
	models := p.ListModels()
	if len(models) == 0 {
...
func TestProviderNameConstant(t *testing.T) {
	if ProviderName != "scnet" {
		t.Errorf("expected ProviderName 'scnet', got %q", ProviderName)
	}
	if DefaultBase != "https://api.scnet.cn/v1" {
		t.Errorf("expected DefaultBase 'https://api.scnet.cn/v1', got %q", DefaultBase)
	}
}
