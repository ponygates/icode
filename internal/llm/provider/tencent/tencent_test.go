...
func TestNew(t *testing.T) {
	...
	if !ok {
		t.Fatal("Tencent provider should wrap openai_compat.BaseProvider")
	}
...
func TestNew_CustomBase(t *testing.T) {
	customBase := "https://custom.tencent.com"
...
func TestDefaultModels(t *testing.T) {
	...
	if !ids["hunyuan-turbos"] {
		t.Error("expected hunyuan-turbos in default models")
	}
	if !ids["hunyuan-t1"] {
		t.Error("expected hunyuan-t1 in default models")
	}
}
...
func TestProviderViaNew(t *testing.T) {
	p := New("sk-test", "")
	if p.Name() != "tencent" {
		t.Errorf("expected name 'tencent', got %q", p.Name())
	}
	models := p.ListModels()
	if len(models) == 0 {
...
func TestProviderNameConstant(t *testing.T) {
	if ProviderName != "tencent" {
		t.Errorf("expected ProviderName 'tencent', got %q", ProviderName)
	}
	if DefaultBase != "https://api.hunyuan.cloud.tencent.com/v1" {
		t.Errorf("expected DefaultBase 'https://api.hunyuan.cloud.tencent.com/v1', got %q", DefaultBase)
	}
}
