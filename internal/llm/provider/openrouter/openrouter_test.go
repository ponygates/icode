...
func TestNew(t *testing.T) {
	...
	if !ok {
		t.Fatal("OpenRouter provider should wrap openai_compat.BaseProvider")
	}
...
func TestNew_CustomBase(t *testing.T) {
	customBase := "https://custom.openrouter.ai"
...
func TestDefaultModels(t *testing.T) {
	...
	if !ids["openrouter/auto"] {
		t.Error("expected openrouter/auto in default models")
	}
...
func TestProviderNameConstant(t *testing.T) {
	if ProviderName != "openrouter" {
		t.Errorf("expected ProviderName 'openrouter', got %q", ProviderName)
	}
	if DefaultBase != "https://openrouter.ai/api/v1" {
		t.Errorf("expected DefaultBase 'https://openrouter.ai/api/v1', got %q", DefaultBase)
	}
}
