...
// entries only exist so they show up in the autocomplete dropdown.
  ...
  // More backend-implemented commands (slashui vocabulary) surfaced so they
  // appear in autocomplete / /help — the server runs them via /api/slash.
  { name: 'fork', usage: '<id>[@n]', descKey: 'slash.descFork', run: () => ({ type: 'passthrough' as const }) },
  { name: 'restore', usage: '<id>', descKey: 'slash.descRestore', run: () => ({ type: 'passthrough' as const }) },
  { name: 'provider', usage: '[id]', descKey: 'slash.descProvider', run: () => ({ type: 'passthrough' as const }) },
  { name: 'apply', descKey: 'slash.descApply', run: () => ({ type: 'passthrough' as const }) },
  { name: 'reject', descKey: 'slash.descReject', run: () => ({ type: 'passthrough' as const }) },
  { name: 'lsp', usage: '<status|diag|syms|hover|def|refs>', descKey: 'slash.descLsp', run: () => ({ type: 'passthrough' as const }) },
  { name: 'kb', usage: '<query>', descKey: 'slash.descKb', run: () => ({ type: 'passthrough' as const }) },
  { name: 'idle', usage: '<名称> <描述>', descKey: 'slash.descIdle', run: () => ({ type: 'passthrough' as const }) },
  { name: 'tasks', descKey: 'slash.descTasks', run: () => ({ type: 'passthrough' as const }) },
  { name: 'mesh', descKey: 'slash.descMesh', run: () => ({ type: 'passthrough' as const }) },
  { name: 'thinking', usage: '[on|off]', descKey: 'slash.descThinking', run: () => ({ type: 'passthrough' as const }) },
  { name: 'skill-eval', usage: '<名称>', descKey: 'slash.descSkillEval', run: () => ({ type: 'passthrough' as const }) },
