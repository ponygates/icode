...
## v0.53.8 — 勾选清单并入内置目录（修复「列表里没有、手动加又说已存在」）+ 搜索筛选框 (2026-09-10)

> 用户反馈：GLM 的 `glm-4.7-flash` 在「获取模型」清单里找不到，但手动添加时又提示「已内置」。两条提示互相矛盾，根因是同一处——清单只来自厂商 `/models`，内置目录从没并进去。

- **【真实 bug】清单与添加接口口径不一致**：`/api/models/fetch` 只返回厂商 `/models` 报的内容（外加用户手工加的模型），**不含内置目录**；而手动添加走 `cataloguedByProvider` 直接 409「已内置模型 X」。于是「厂商没报、但本 build 认识」的内置模型**在两条路上都够不着**：清单里看不见，手工加被拒。
- **【更严重的连带后果】勾选保存会把它们永久挤出模型列表**：保存时 `SetEnabledModels` 存的是清单里勾中的 id 集合；厂商没报的内置模型不在集合里 → 保存后 `/api/models` 的过滤器把它挡掉 → **模型从列表里消失且再也加不回来**（回到上一条的死路）。这正是「大模型里还是有的加不进去」的真身。
- **修法一：清单改为三方并集** —— 厂商实时列表 + 内置目录 + 用户自定义，厂商自己的列表排在最前。内置目录补充进来的条目带 `builtin_only` 标记，前端显示「内置」徽标，表头说明「已并入 N 个厂商列表未返回的内置模型」——否则多出来的行看起来像 bug。
- **修法二：内置模型在清单里显示目录数值** —— 内置模型在 `/api/models` 里用的是注册表条目，所以清单必须显示同一组数字（名称/上下文窗口/最大输出），否则清单会承诺一个 app 实际不使用的窗口。
- **修法三：手动添加内置模型不再是死路** —— 仍**不**创建 custom 条目（空条目会遮蔽带套餐/定价/窗口的完整条目，这条理由不变），但改为**把它并入该厂商的启用集合并返回 200**。唯一仍然报错的情况是厂商开关本身关着（那时确实无法可见），错误文案也改成可操作的一句话。
- **新功能：勾选清单加搜索筛选框** —— 按 id 与显示名称不区分大小写匹配；有查询时显示「显示 X / Y」，无匹配给出空态而不是一块空白。
- **【同源 bug】「全选」原本是覆盖式，会静默丢掉被筛掉的勾选** —— 改为**并集**语义：勾选只增不减，取消只移除给定 id，同时清掉列表中已不存在的陈旧 id。否则在筛选状态下点全选会清空隐藏项的勾选，而这份勾选正是保存时要写进厂商过滤器的内容。按钮作用域也随之变为当前可见行，文案相应区分（全选/取消筛选结果）。
- **新增测试 12 项**：服务端 4（内置模型可脱离厂商过滤器 / 厂商停用时仍诚实报错 / 清单含厂商遗漏的内置模型且带 `builtin_only` 与目录数值 / 「全选后保存」不再丢内置模型）+ 前端 8（面板 5：搜索过滤、名称匹配、无匹配空态、全选作用域与隐藏勾选保留、内置徽标与说明；hook 3：子集并集不重置、子集取消只删指定 id、清理陈旧 id）。另有 1 项按新契约重写（原「重复添加内置模型返回 409」→ 现在断言它被启用且内置条目未被遮蔽）。
- 验证：`go build` / `go vet` / `gofmt` / `go test ./...` 全绿；`tsc --noEmit` 零错误、`vitest run` 99 项全通过。

## v0.53.7 — 子代理继承每模型生成参数（补上 v0.53.6 覆盖层的最后一个缺口）(2026-09-10)

> v0.53.6 把「按模型解析生成参数」的覆盖层接进了引擎的**三个**请求点，但子代理走的是另一条链路：`agent.Runner` 自建 `ChatRequest`，温度**硬编码 0.1**。于是用户把某个模型设成「精确」或在 ⚙️ 里调了 top_p，主对话生效、Task / 会话 fork 出来的子代理却纹丝不动。

- **`agent.Runner` 新增每模型参数解析**：`ParamsResolver` 类型 + `SetModelParamsResolver` + 纯函数 `resolveGeneration`（纯函数便于直接单测，无需起 provider）。子代理默认温度仍是 0.1（低温度让工具调用参数更稳定，这是原有设计），只有用户真的为该模型配了值才覆盖——**包括显式 0**。
- **为什么不接管 MaxTokens**：`AgentDef.MaxTokens`（默认 4096，个别代理 2048/8192）是**按代理的设计预算**而非按模型的；用模型级 `max_tokens` 覆盖它会破坏代理定义，因此只接 `temperature`/`top_p`，并在代码注释里写明理由。
- **引擎在懒创建 runner 时注入解析器**：`getAgentRunner` 里一并 `SetModelParamsResolver`。runner 在首次派发 Task 时才创建，晚于 `server.New` / `app.go` 的注入，因此此处赋值即生效。新增 `Runner.HasModelParamsResolver()` 供主机与测试确认接线真的到位——**上一次 provider 重试静默失效的教训：这类跨层接线必须可被断言验证，不能只看代码"看起来"接上了**。
- **新增测试 8 项**：agent 包 6（默认 0.1 / 覆盖生效 / **显式 0 被尊重** / 未知模型保留默认 / 只设 top_p / 解析器装没装可查询）、conversation 包 2（引擎确实把解析器转发给了子代理 runner / 未配置时不虚报）。
- 验证：`go build` / `go vet` / `gofmt -l` / `go test ./...` 全绿。本版无前端改动，未重跑 `tsc`。
...
### 一、在厂商卡片里自定义增加 / 删除模型
- **新增模型**：每个厂商卡片内新增「为此厂商添加模型」按钮——厂商由点击位置隐含，不必再手打厂商名；共用组件 `components/AddModelModal.tsx` 在设置弹窗与独立模型页两处复用，避免两份实现漂移。工具栏的「添加模型」仍可自由指定任意厂商（用于尚未渲染卡片的厂商）。
- **【真实 bug】「点了新增没反应」**：厂商已被「获取模型」收窄过（`EnabledModels` 非空）时，新模型**存进了配置却被过滤器挡在列表外**。修法：新增后调 `ensureModelEnabled` 把新 id 并进该厂商的过滤器。
- **【真实 bug】删除从来没成功过**：前端拼 key 时把已是 `provider/model` 形式的 id 又加了前缀，得到 `provider/provider/model` → DELETE 404。修法：按 `startsWith` 归一化 canonical id。
- **【真实 bug】删除留下悬挂的过滤 id**：删掉一个 custom 模型后，它的 id 仍留在 `EnabledModels` 里，把厂商锁死在一个已不存在的模型上。修法：删除前先读原条目，删后 `forgetModelEnabled` 清理（若因此清空则回到「不限制」，语义一致）。
- **加内置已有模型 → 409**：此前会写入一条空描述条目，把带套餐/定价/上下文窗口的完整内置条目**覆盖掉**。现在拒绝并提示「已内置，无需重复添加」。
- 错误信息透传：解析 `detail.error`，不再只显示裸 `HTTP 409`。
- **决策：不伪造 BaseURL 字段**。核实 `ModelCfg.BaseURL` 在 Go 侧**从未被读取**（死字段），表单里那个 "API Base URL" 是空的；选择在 UI 里如实说明「模型走所属厂商的 endpoint，要换端点请先添加厂商」，而不是提供一个无效输入框。

### 二、非对话模型被当成对话模型上架（就是「少数模型用不了」）
- `fetchOpenRouterModels` 的注释写着「Only include models that support tool calling」，**但代码里没有任何过滤**——注释描述的是一个从未实现的意图（属于「注释撒谎」类问题，比没注释更误导）。
- `fetchOpenAICompatModels` 给**每一个**返回项都盖上 `Tools: true, Streaming: true`，包括 `text-embedding-3-small`、`whisper-1`、`tts-1`、`dall-e-3`、`omni-moderation-latest` 这类根本不能做对话的条目。
- 修法：新增 `internal/llm/modelmeta` 包集中做这件事，两条摄取链路（`openai_compat.FetchModels` 实时获取 / `pkg/modelupdate` 刷新管道）**共用同一个过滤器与解析器**，不再各写一份。
- 过滤是**按 id 的子串启发式**，且刻意偏保守：漏掉一个只是列表里多一条无用项，误杀一个却会把**能用**的模型从用户眼前藏起来——后者是更坏的失败。`gpt-4o`（带视觉）、`deepseek-reasoner`、`qwen2.5-coder` 等正常对话模型均不受影响。

### 三、厂商自报的元数据以前被丢弃，现在自动生效（「根据厂家的设置自动调整上下文长度和其它设置」）
- 厂商在 `/models` 里**确实会说**上下文窗口与最大输出（OpenRouter 用 `context_length` + `top_provider.max_completion_tokens`，不少网关用 `context_window`/`max_output_tokens`/`max_completion_tokens`，还有的写成字符串 `"128k"`）。这些字段此前**全部被忽略**，新模型只能拿到一个硬编码的 128K/8K，或者干脆是 0（UI 显示空白，上下文占用率退回以 200000 为分母的兜底）。
- 现在统一由 `modelmeta.ParseVendorList` 解析：数字、字符串、`"128k"` 后缀都能吃；**厂商没说就留 0，绝不猜**——本项目的原则是不臆造规格参数。
- 优先级：内置目录已有的模型 → 用内置元数据；厂商新报的模型 → 用**厂商自己报的**数值；厂商沉默 → 才退回保守默认值。
- **勾选保存时一并回传**：`PUT /api/models/selection` 新增可选 `meta` 字段，自动注册为 custom 的模型会**带着厂商报的上下文窗口落盘**（此前注册出来是空的）。前端 `useModelFetch` 只为**被勾选**的模型回传，其余本就要被排除。
- **顺带**：`modelmeta` 的解析器兼容三种 `/models` 形态（`{"data":[...]}` / `{"models":[{"name"}]}` Ollama / 裸数组），去重保序；`parseVendorModelIDs` 保留为薄封装，既有调用与测试不受影响。
- **已知未修（已诊断）**：每模型 ⚙️ 面板的「保存设置」会把 `temperature`/`max_tokens`/`top_p` 发给 `PUT /api/config/key`，而该接口只解码 provider/api_key/api_base，这些参数被静默丢弃，界面却仍显示「已保存」。改成真正的「每模型生成参数覆盖」需要给引擎注入按模型解析的覆盖层（引擎当前只有一对全局参数，且不持有 config），属触及对话主链路的非平凡改动，另行评估。

- **新增测试 29 项**：`modelmeta` 12（非对话识别 13 例、误杀防护 11 例、保序过滤、四种形态、字符串与 `128k` 数值、去重与空 id、垃圾输入返回空、厂商沉默时**不猜**）、后端 7（新增可见/未收窄新增/重复内置 409/空白 id 拒绝/删除清理过滤项/删空恢复目录/厂商元数据落盘）、前端 10（`AddModelModal` 9 + 保存体含 `meta` 断言 1，并更新 2 处既有断言）。
- 验证：`go build` / `go vet` / `gofmt` / `go test` 全绿；前端 `tsc --noEmit` 零错误、`vitest run` **80/80**；四份二进制重编，PE 3/3/2/2，内嵌 chunk `ModelsPage-gJMdjQIu.js` 各唯一。

## v0.53.4 — 全部厂商一键获取 + 模型排序（可用性打磨）(2026-09-10)

> v0.53.3 的「获取模型」是逐个厂商点的。这一轮补上批量获取与列表排序，并顺手修掉一个让上下文窗口显示长期失真的潜伏 bug。

- **【真实 bug】上下文窗口一直是错的**：`/api/models` 的 DTO 只发蛇形 `context_window`/`max_output_tokens`，而桌面渲染层读的是驼峰 `model.contextWindow`/`model.maxOutputTokens` —— 双方从未对上，于是：
  - `ChatPage` 的上下文占用率**恒以 200000 为分母**（`currentModel?.contextWindow || 200000` 的兜底永远生效），128K 的模型和 1M 的模型显示完全一样；
  - `ModelsPage` 的「📐 N K ctx」标签**从未渲染过**。
  修法：DTO 同时输出两种拼写（纯新增字段，不破坏任何既有消费方）。这个问题是给「按上下文长度排序」写代码时发现的——排序原本会按全 0 排，退化成空操作。
- **`known` 字段此前是常量 `true`**：`/api/models/fetch` 无条件上报 `known: true`，信息量为零。改为真实比对内置目录，UI 才能标出「厂商已提供但本构建不认识」的模型——这正是实时获取存在的意义。
- **全部厂商一键获取（并发限流 3）**：`useModelFetch.fetchAll(providers, concurrency=3)` 用共享游标跑并发池（JS 单线程 + 取任务之间无 await，故两个 worker 不可能抢到同一厂商）。
  - **结果分三类而非一类**：`ok` / `unsupported`（后端 501，该厂商没有实时发现端点）/ `failed`（无 Key、Key 被拒、网络）。把「这家厂商不支持」和「你的密钥被拒」合并成一个错误数会误导排查方向。
  - 强制绕过会话缓存（「要最新的」就是它的全部意义）；并发上限 3 是折中——全串行在慢厂商上要几分钟，无上限则会撞限流（OpenRouter 单家目录就有数 MB）。
  - **重入保护用 `useRef` 而非 state**：state 要等下次渲染才可见，同一 tick 内的两次点击会双双通过守卫、对同一批厂商跑两遍。ref 同步翻转，才是真守卫。
  - UI：两个模型页都加「全部获取」按钮 + 进度条 + 「成功 N · 不支持 N · 失败 N」分项汇总（零值不显示，可关闭）。
- **模型列表排序**：默认顺序 / 上下文长度 ↓ / ↑ / 新发现优先。抽成纯函数 `desktop/src/lib/modelSort.ts` 便于单测。
  - **缺失值两个方向都垫底**：把「未知」当 0 会让它在升序里冲到最前，用大哨兵值又会在降序里冲到最前——要么都是一屏空白。未知不是最小，是「没有」。
  - 「新发现优先」按 `custom` 分组，并利用 `Array#sort` 的稳定性保持组内目录顺序（否则每次渲染都乱序）。
  - 模型行新增「✦ 新」标记（tooltip 说明来源），勾选面板里同样标记 `known: false` 的条目。
- **新增测试 23 项**：`modelSort` 8（方向/未知垫底/稳定性/空列表/零与负值视为未知/不可变）、`useModelFetch` 7（三类结果分类、并发上限实测峰值 = 3、绕过缓存、同 tick 重入返回 null 且只发 4 个请求、结束后无残留进度、关闭汇总、空列表）、`ModelFetchAllBar` 7（触发参数、无厂商禁用、运行中禁用并显示进度、分项汇总、零值隐藏、关闭、可重跑）、后端 4（驼峰字段接线 + 蛇形兼容、`known` 真实性、两项既有集成）。
- 验证：`go build` / `go vet` / `gofmt` / `go test` 全绿；前端 `tsc --noEmit` 零错误、`vitest run` **71/71**。顺带修复 2 处测试文件的类型问题（`global` → `globalThis`、可能为 undefined 的断言）——`vite build` 不做类型检查，所以上一轮漏网，这次 `tsc` 抓到了。

## v0.53.3 — 一键获取厂商模型 + 勾选启用（功能/智能对齐）(2026-09-10)

> 内置模型目录只是**编译时的快照**：厂商每天都在上架/下线模型，而某个 Key 到底能用哪些模型还要看套餐与地域。此前只有 `RefreshAll`（仅健康检查）和 `/api/models/refresh`（只查内置目录），**没有任何实时拉取能力**。本轮补上「厂商才是权威」这条链路。

- **实时获取模型（`types.ModelFetcher` 可选能力接口）**：
  - `openai_compat.BaseProvider.FetchModels` 调 `apiBase + /models`，带已存 Key 与自定义头；解析器同时兼容 `{"data":[{"id"}]}`（OpenAI 方言）、`{"models":[{"name"}]}`（Ollama 方言）与裸数组，去重保序。命中内置目录的模型**复用其上下文窗口/最大输出等元数据**，新模型用保守默认（128K / 8K）。
  - Anthropic 侧独立实现（`/models?limit=1000`，取 `display_name`，默认 200K 上下文）。
  - **13 家厂商全覆盖**：12 家 OpenAI 兼容厂商经 `openai_compat.NewProvider` 构造，自动获得该能力；未实现者返回 501 + 明确文案，前端如实提示而非静默失败。
  - ⚠️ 拉取会消耗一次厂商往返，OpenRouter 目录可达数 MB，故服务端强制 30s 超时，前端逐个厂商串行拉取（同一时刻只允许一个请求在途）。
- **按厂商勾选启用（`ProviderCfg.EnabledModels`）**：
  - `GET /api/models/fetch?provider=X` 返回该 Key 可用清单 + 每项当前是否启用；`PUT /api/models/selection` 持久化勾选结果。
  - **勾选清单里未出现在内置目录的模型会自动注册为 custom 模型**——厂商新发布的模型无需等 iCode 发版即可选中并使用。
  - **向后兼容**：`EnabledModels` 为空 = 不设限制（`IsModelEnabled` 返回 true），从未用过该功能的用户行为完全不变。
  - **拒绝空选择**：空过滤器含义是「不限制」，会反过来把整个目录放开——与用户意图完全相反。空勾选返回 400 并提示改用厂商开关停用整个厂商。
  - 拉取结果会**并入用户手工添加的 custom 模型**，否则「全选」会悄悄丢掉手工加的模型。
- **UI（桌面）**：设置弹窗「模型」页与独立「模型」页的每个厂商卡片都新增「获取模型」按钮（带加载态与 `spin` 动画）；点击展开勾选面板——全选/全不选、上下文长度、原始模型 ID、已选计数、应用；卡片显示「已启用 N/M」徽标。逻辑与 UI 抽为 `lib/useModelFetch.ts` + `components/ModelFetchPanel.tsx` 两页共用，避免两份实现漂移。
  - 独立模型页现在也会列出**尚无模型的自定义厂商**（此前因「无模型即隐藏」而永远无法进入、也就永远无法获取模型）——这正是「获取模型」要填的空目录。
- **新增测试 28 项**：后端 `openai_compat` 8（解析三方言/去重/复用元数据/缺 Key/401 透传/空目录报错）、`config` 5（无过滤全放开、过滤行为、带前缀 ID 匹配、`VendorModelID`、清空过滤）；前端 `useModelFetch` 12（预勾选策略、缓存复用、二次点击收起、只提交勾选项、空选择拦截、后端错误透传、空目录、保存失败不关面板、保存后失效缓存）、`ModelFetchPanel` 3（渲染勾选项、取消勾选后提交缩减集合、全不选禁用提交）。
- 验证：`go build ./...` / `go vet ./...` / `go test ./...` 全绿（EXIT=0）；前端 `tsc --noEmit` 零错误、`vitest run` 49/49 通过。
...
## v0.52.4 补丁2（2026-09-06）

- **根因修复：流式期间的「⏳ 输入中」平行缓冲行退役**。此前生成中打字进的是 queueBuf（显示在输入框上方一行）——免费模型流式又慢，用户以为是普通打字，观感即「文字交替跑到上面」。现在：生成中打字直接进**输入框**（光标/退格全可用），**Enter** 才把内容排队（⏳ 行只显示已排队条目），↑ 召回也进输入框。与桌面端行为一致。
- **ICODE_TRACE=1 帧追踪**（上轮）：渲染错位类问题的取证工具。

## v0.52.4 补丁2（2026-09-06）

- **ICODE_TRACE=1 帧追踪**：设置该环境变量后，TUI 把每帧所有绝对定位写（行/列+内容预览）记录到 ~/.icode/tui-trace.log——「文字渲染错位」类问题的决定性证据工具。

## v0.52.4 补丁（2026-09-06）

- **ConHost 扩展键全面翻译**：按键泵把「0xE0 前缀+扫描码」翻译为 VT 序列——↑↓←→、Home/End、**Delete**、Insert、PgUp/PgDn 全部不再漏成字母乱码（此前 Delete 显示为「S」、Home 为「G」等）。
- **Delete 键生效**：前向删除光标处字符（带 undo），此前该键被静默吞掉。
- 未知扩展键静默丢弃（F1-F12 等后续按需补）。

## v0.52.3 补丁（2026-09-06）

- **CLI 光标漂移根治**：整帧改单次 flush——主画面与输入框/覆盖层合并为一次 write，消除帧中间光标停留在消息区的窗口期（IME 组合与输入不再落到上方空白处）。
- **桌面启动 LOGO 还原梅花**（BootSplash 恢复 PlumBlossom 品牌标）。

## v0.52.2 三补 — 审查收官（2026-09-05）

- **主题跨端共享**：桌面主题设置直写后端 config（tui.theme），启动时无本地选择则采纳后端值——TUI/simpleui/桌面三端一致。
- **xgo 冒烟测试**：GoSafe panic 恢复 + 正常执行两用例。
- **ACP e2e 扩展至 9 用例**：新增 session/prompt 无密钥终止路径与 session/cancel 通知不崩验证。
- **修复关键回归**：审查项落地时 SetMaxToolRounds 接线被放在 NewEngine 之前——Engine 为 nil 导致所有入口启动即 panic。已移到创建后并加注释守卫。
- 至此 docs/项目审查_2026-08-31.md 的 24 项发现全部闭环（21 项修复 + 3 项验证为优保留）。

## v0.52.2 再补 — 批次三/四收官（2026-08-31）

- **Ctrl+R 升级搜 transcript**：搜索范围从输入历史扩展到整段会话记录（用户+助手+系统消息，新→旧，去重），历史兜底。
- **simpleui 状态段**：⎇ git 分支（30s 缓存）+ ⏱ 回合计时接入底部状态条。
- 审查报告 24 项已完成 21 项；剩余 3 项（主题跨端共享、desktop/xgo 冒烟测试、ACM 尾巴）转入下轮。
...
- **命令补全可循环**：↑ 在第一项继续按跳到最后一项，↓ 在最后一项跳回第一项。
- **Tab 改为接受补全/循环权限模式**：不再切模型（模型有 /model、Alt+P 与选择器）。
- **Ctrl+, 设置面板可用**：Enter 按 行生效——模型→选择器、模式→循环、语言→循环三语、主题→明暗切换、供应商/语音凭据→打开完整配置编辑。
- **英文输入丢首键**：按键泵过滤 NUL/0xE0 扩展键前缀，UTF-16 代理对（表情等星面字符）成对合并，不再有残留半字符。
...
- **`SessionViewer.tsx`**：只读会话对照面板——复用 `Markdown` 组件渲染另一会话消息（User 蓝 / Assistant 灰气泡），无输入、无流式、零交互状态依赖。
- **入口**：右侧侧边栏顶部「分屏对照」下拉——选另一会话即并排显示（42% 宽，边框分隔），标题带 👁 标识 + 关闭按钮；选当前会话自动隐藏。
- **三语 i18n**：split.title/off/viewer/empty/close。
- 验证：tsc 0 错；vite build ✓；Go 全量测试绿；四份二进制重编 PE 正确（含嵌入新前端）。
- **D1 完整状态**：并行流式 + TabBar 拖拽（v0.48.0）→ SplitPane 组件（v0.50.0）→ 分屏对照（本版）。双交互分屏留二期（清单已勘明）。
...
- **D4 技能市场 +3（现 18 个）**：
  - `docx-report`（Word 报告：保单检视报告/财务规划建议书，pandoc 或 python-docx，中文排版规范 + 合规免责）
  - `xlsx-sheet`（Excel 表格：客户信息表/保单清单/对比表，pandas 或 openpyxl，**数据脱敏**：姓名代号、手机号打码、不上传）
  - `pptx-deck`（幻灯片：展业演示/培训课件，python-pptx，封面+要点页+行动号召，配色简洁）
  - 三个均贴合保险展业场景，含「不虚构费率/收益、以条款为准」合规约束。
- **D7 首跑向导「测试连接」按钮**：填 key 后一键验证（保存 key → 调 `/api/models` 拉取验证），成功/失败即时反馈，消除「填错 key 完成向导才发现」的痛点；三语 i18n。
- 验证：tsc 0 错；vite build ✓；Go 全量测试绿；四份二进制重编 PE 正确（含嵌入新前端）。

> 差距表 C/D 项收官：C1 C2 C4 ✅、D1(部分) D2 D3 D5 D6 D7 ✅、D4 ✅。**C3 ACP（编辑器协议接入）+ D1 分屏**为大工程（协议适配层 / 双面板），建议专项迭代。
