...
function cfg(key, def) {
    return vscode.workspace.getConfiguration('icode').get(key, def);
}
...
async function ensureBackend() {
    ...
    if (!cfg('autoStartBackend', true)) {
        throw new Error('iCode 后端未运行，且已禁用自动启动（icode.autoStartBackend=false）。请先运行 `icode server`。');
    }
    startBackend();
    for (let i = 0; i < 60; i++) {
    ...
    updateStatusBar(null);
    throw new Error('iCode 后端未能启动。请确认已安装 iCode (https://github.com/ponygates/icode)，或先在终端运行 `icode server`。');
}
function findIcodeBin() {
    // User-configured path takes priority.
    const configured = cfg('binPath', '');
    if (configured) {
...
function startBackend() {
    ...
    if (!bin) {
        vscode.window.showErrorMessage('未找到 icode 可执行文件。请先安装 iCode: https://github.com/ponygates/icode');
        return;
    }
...
function proxyChat(port, body, webview, sessionId) {
    ...
    const req = http.request({
        ...
        res.on('error', (e) => webview.postMessage({ type: 'error', sessionId, message: String(e) }));
    });
...
class ICodeExtension {
    ...
    async sendSelectionToChat(instruction, autoSend) {
    ...
    async activate(ctx) {
        ...
        ctx.subscriptions.push(vscode.window.registerWebviewViewProvider('icode.chat', provider, {
        ...
        ctx.subscriptions.push(vscode.commands.registerCommand('icode.openSidebar', () => {
